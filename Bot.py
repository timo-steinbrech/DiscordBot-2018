...
#async def on_message(message):
        ...
        #else:
                #await client.send_message(message.channel, "You dont have Power here")

@client.event
async def on_ready():
    await client.change_presence(game=discord.Game(name='mit der Matrix'))
    print("Bruder muss los dieser Bot ist gnadenlos")
...
async def help(ctx):
    ...
    embed.add_field(name='!fortnite', value='Meinung über Fortnite', inline=False)
...
@client.command()
async def spaß():
    await client.say('Ich hab Spaß ich hab Gas')


@client.command()
async def ehre():
    await client.say('Ehre genommen, Vorhaut genommen. Leben genommen')
...
@client.command()
async def fortnite():
    await client.say('Fortnite ist scheiße')
